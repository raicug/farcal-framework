...
void format_slider_value(char* buffer, std::size_t buffer_size, double value, bool integral, std::string_view suffix)
{
    if (integral) {
        if (suffix.empty()) {
            ...
            std::snprintf(buffer, buffer_size, "%.0f %.*s", value, static_cast<int>(suffix.size()), suffix.data());
        }
    } else {
        if (suffix.empty()) {
            std::snprintf(buffer, buffer_size, "%.2f", value);
        } else {
            std::snprintf(buffer, buffer_size, "%.2f %.*s", value, static_cast<int>(suffix.size()), suffix.data());
        }
    }
}
