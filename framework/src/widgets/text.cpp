...
void Text(std::string_view value) {
  ...
  const Rect bounds = widget_internal::next_rect(
      widget_internal::text_bounds_width(value), Height);
  ...
  widget_internal::set_last_item(
      ...
      widget_internal::contains(widget_internal::active_clip(bounds),
                                Input().MousePosition),
      false, widget_internal::focused_item == CurrentId(value));
}
...
void SectionText(std::string_view value) {
  ...
  const Rect bounds = widget_internal::next_rect(
      widget_internal::text_width(value), 20.0F * theme.FrameScale);
  widget_internal::add_text(bounds, value, theme.TextSecondary);
