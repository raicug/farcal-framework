...
bool BeginList(std::string_view label, Vec2 size) {
  ...
  widget_internal::layout.ContentWidth =
      std::max(0.0F, inner.Max.X - inner.Min.X);
...
bool ListItem(std::string_view label, bool selected) {
  ...
  const Rect bounds =
      widget_internal::next_rect(widget_internal::layout.ContentWidth > 0.0F
                                     ? widget_internal::layout.ContentWidth
                                     : theme.ItemWidth * scale,
                                 height);
  const std::uint64_t id = CurrentId(label);
  ...
  const Color fill =
      ...
      : selected ? Color::RgbaF(theme.Selection.R, theme.Selection.G,
                                theme.Selection.B, item.Hovered ? 0.68F : 0.48F)
      : item.Hovered
          ? Color::RgbaF(theme.ButtonHovered.R, theme.ButtonHovered.G,
                         theme.ButtonHovered.B, 0.88F)
          : Color::RgbaF(theme.Button.R, theme.Button.G, theme.Button.B, 0.0F);
  const Color text_color = selected || item.Hovered || item.Active
                               ? theme.Text
                               : theme.TextSecondary;
  ...
  widget_internal::add_text(
      {{bounds.Min.X + 8.0F * scale, bounds.Min.Y}, bounds.Max}, label,
      text_color);
