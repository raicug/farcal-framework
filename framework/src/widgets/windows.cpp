...
bool BeginWindow(std::string_view Title) {
  ...
  widget_internal::window_state &state =
      widget_internal::windows[std::string(Title)];
  ...
  const Rect title_bounds{bounds.Min,
                          {bounds.Max.X, bounds.Min.Y + title_height}};
  const bool Hovered = widget_internal::contains(bounds, io.MousePosition);
  const bool title_hovered =
      widget_internal::contains(title_bounds, io.MousePosition);
  ...
  const Rect current_title_bounds{bounds.Min,
                                  {bounds.Max.X, bounds.Min.Y + title_height}};
  const Rect content_bounds{{bounds.Min.X + 1.0F, bounds.Min.Y + title_height},
                            {bounds.Max.X - 1.0F, bounds.Max.Y - 1.0F}};
  const Rect scroll_clip{
      {bounds.Min.X + 18.0F * scale,
       bounds.Min.Y + title_height + 16.0F * scale},
      {bounds.Max.X - 18.0F * scale, bounds.Max.Y - 16.0F * scale}};
  const Rect shadow_1{
      {bounds.Min.X + 3.0F * scale, bounds.Min.Y + 4.0F * scale},
      {bounds.Max.X + 3.0F * scale, bounds.Max.Y + 4.0F * scale}};
  const Rect shadow_2{
      {bounds.Min.X + 1.0F * scale, bounds.Min.Y + 2.0F * scale},
      {bounds.Max.X + 1.0F * scale, bounds.Max.Y + 2.0F * scale}};

  widget_internal::add_soft_rect(shadow_1, 4.0F * scale,
                                 {0.0F, 0.0F, 0.0F, 0.18F});
  widget_internal::add_soft_rect(shadow_2, 4.0F * scale,
  ...
  widget_internal::add_soft_rect(current_title_bounds, 4.0F * scale,
                                 Hovered ? theme.WindowTitleActive
                                         : theme.WindowTitle);
  widget_internal::add_filled_rect(
      {{current_title_bounds.Min.X, current_title_bounds.Max.Y - 4.0F * scale},
       current_title_bounds.Max},
      Hovered ? theme.WindowTitleActive : theme.WindowTitle);
  widget_internal::add_hline(bounds.Min.X, bounds.Max.X,
                             current_title_bounds.Max.Y,
                             {theme.WindowBorder.R, theme.WindowBorder.G,
                              theme.WindowBorder.B, 0.70F});
  widget_internal::add_soft_outline(bounds, 4.0F * scale, theme.WindowBorder);
  widget_internal::add_text(
      {{current_title_bounds.Min.X + 10.0F * scale, current_title_bounds.Min.Y},
       current_title_bounds.Max},
      Title, theme.Text);
  ...
  widget_internal::window_stack.push_back({
      ...
      .MaxScroll = MaxScroll,
      .Hovered = Hovered,
      .ScrollTrack = {{scroll_clip.Max.X - 7.0F * scale, scroll_clip.Min.Y},
                      {scroll_clip.Max.X - 3.0F * scale, scroll_clip.Max.Y}},
  });
  ...
  widget_internal::layout.ContentWidth =
      scroll_clip.Max.X - scroll_clip.Min.X - 18.0F * scale;
...
void EndWindow() {
  if (widget_internal::layout_stack.empty() ||
      widget_internal::window_stack.empty()) {
    throw std::logic_error("farcal Window stack is empty");
  }

  const widget_internal::window_layout_state window_layout =
      widget_internal::window_stack.back();
  widget_internal::window_stack.pop_back();

  widget_internal::window_state &state =
      widget_internal::windows[window_layout.Title];
  state.ContentHeight = std::max(0.0F, widget_internal::layout.Cursor.Y -
                                           window_layout.ContentStartY +
                                           window_layout.ContentPaddingY);
  ...
  if (window_layout.MaxScroll > 0.0F) {
    ...
    const float thumb_height =
        std::max(28.0F * theme.FrameScale,
                 track_height * std::min(1.0F, window_layout.VisibleHeight /
                                                   state.ContentHeight));
    const float thumb_y =
        track.Min.Y + (track_height - thumb_height) *
                          (state.ScrollY / window_layout.MaxScroll);
    ...
    widget_internal::add_soft_rect(thumb, 2.0F * theme.FrameScale,
                                   {theme.TextMuted.R, theme.TextMuted.G,
                                    theme.TextMuted.B,
                                    window_layout.Hovered ? 0.70F : 0.42F});
  }
