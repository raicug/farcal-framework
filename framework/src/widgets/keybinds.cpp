...
float measured_text_width(std::string_view value, const Style& theme, float scale)
{
    if (value.empty()) {
        return 0.0F;
    }
...
std::string_view key_name(int value)
{
    ...
    const UINT scan_code = MapVirtualKeyA(static_cast<UINT>(value), MAPVK_VK_TO_VSC);
    if (scan_code != 0) {
        ...
        const int length = GetKeyNameTextA(parameter, buffer.data(), static_cast<int>(buffer.size()));
        if (length > 0) {
...
bool mode_popup(std::uint64_t id, Rect value_rect, KeybindMode* mode)
{
    ...
    if ((io.MousePressed[0] || io.MousePressed[1]) && !widget_internal::contains(popup, io.MousePosition) && !widget_internal::contains(value_rect, io.MousePosition)) {
        open_keybind_menu = 0;
    }

    menu_filled_rect(popup, Color::RgbaF(theme.WindowPanel.R, theme.WindowPanel.G, theme.WindowPanel.B, 0.98F));
    menu_hline(popup.Min.X + 1.0F, popup.Max.X - 1.0F, popup.Min.Y + 1.0F, Color::RgbaF(1.0F, 1.0F, 1.0F, 0.06F));
    menu_outline(popup, Color::RgbaF(theme.WindowBorder.R, theme.WindowBorder.G, theme.WindowBorder.B, 0.92F));
    ...
    for (int index = 0; index < 2; ++index) {
        ...
        const widget_internal::item_result item = menu_behavior(CurrentId(labels[index]), option);
        ...
        if (selected || item.Hovered || item.Active) {
            const Color fill = item.Active ? theme.ButtonActive : item.Hovered ? Color::RgbaF(theme.ButtonHovered.R, theme.ButtonHovered.G, theme.ButtonHovered.B, 0.74F) : Color::RgbaF(theme.Button.R, theme.Button.G, theme.Button.B, 0.42F);
            menu_filled_rect(option, fill);
        }

        if (selected) {
            menu_filled_rect({{option.Min.X + 3.0F * scale, option.Min.Y + 6.0F * scale}, {option.Min.X + 5.0F * scale, option.Max.Y - 6.0F * scale}}, theme.Accent);
        }

        menu_text({{option.Min.X + 11.0F * scale, option.Min.Y}, option.Max}, labels[index], selected || item.Hovered ? theme.Text : theme.TextSecondary);
    }
...
bool Keybind(std::string_view label, int* value, KeybindMode* mode)
{
    ...
    const std::string_view display_text = was_capturing ? "..." : key_name(*value);
    const float display_width = measured_text_width(display_text, theme, scale);
    const float value_width = std::clamp(display_width + 24.0F * scale, 42.0F * scale, 86.0F * scale);
    ...
    const widget_internal::item_result item = widget_internal::item_behavior(id, value_rect);
    const bool activated = !was_capturing && (item.Clicked || widget_internal::keyboard_toggle(id));
    ...
    const Color fill = item.Active ? theme.ButtonActive : item.Hovered || now_capturing ? Color::RgbaF(theme.ButtonHovered.R, theme.ButtonHovered.G, theme.ButtonHovered.B, 0.82F) : theme.Button;
    const Color border = now_capturing || open_keybind_menu == id ? Color::RgbaF(theme.Accent.R, theme.Accent.G, theme.Accent.B, 0.82F) : item.Focused ? Color::RgbaF(theme.Accent.R, theme.Accent.G, theme.Accent.B, 0.44F) : item.Hovered ? Color::RgbaF(theme.WindowBorder.R, theme.WindowBorder.G, theme.WindowBorder.B, 0.88F) : theme.ButtonBorder;
    const Color value_fill = now_capturing ? Color::RgbaF(theme.ButtonPrimary.R, theme.ButtonPrimary.G, theme.ButtonPrimary.B, 0.40F) : Color::RgbaF(theme.WindowPanel.R, theme.WindowPanel.G, theme.WindowPanel.B, 0.88F);

    widget_internal::add_text(label_rect, label, item.Hovered || now_capturing ? theme.Text : theme.TextSecondary);
    widget_internal::add_filled_rect(value_rect, fill);
    widget_internal::add_filled_rect({{value_rect.Min.X + 1.0F, value_rect.Min.Y + 1.0F}, {value_rect.Max.X - 1.0F, value_rect.Max.Y - 1.0F}}, value_fill);
    widget_internal::add_hline(value_rect.Min.X + 1.0F, value_rect.Max.X - 1.0F, value_rect.Min.Y + 1.0F, item.Hovered || now_capturing ? Color::RgbaF(1.0F, 1.0F, 1.0F, 0.13F) : Color::RgbaF(1.0F, 1.0F, 1.0F, 0.05F));
    widget_internal::add_soft_outline(value_rect, 0.0F, border);

    const float text_x = value_rect.Min.X + (std::max)(0.0F, (value_rect.Max.X - value_rect.Min.X - text_width) * 0.5F);
    widget_internal::add_text({{text_x, value_rect.Min.Y}, {text_x + text_width + 1.0F * scale, value_rect.Max.Y}}, text, now_capturing || *value != no_key ? theme.Text : theme.TextMuted);
