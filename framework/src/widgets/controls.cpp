...
bool Button(std::string_view label)
{
    ...
    return widget_internal::button_impl(label, theme.Button, theme.ButtonHovered, theme.ButtonActive, theme.Text);
}

bool PrimaryButton(std::string_view label)
{
    const Style& theme = CurrentStyle();
    return widget_internal::button_impl(label, theme.ButtonPrimary, theme.ButtonPrimaryHovered, theme.ButtonPrimaryActive, {1.0F, 1.0F, 1.0F, 1.0F});
}
...
bool Checkbox(std::string_view label, bool* value)
{
    ...
    const float Width = widget_internal::layout.ContentWidth > 0.0F ? widget_internal::layout.ContentWidth : box_size + gap + label_width;
    ...
    widget_internal::add_filled_rect({{box.Min.X + 1.0F, box.Min.Y + 1.0F}, {box.Max.X - 1.0F, box.Max.Y - 1.0F}}, fill);
    widget_internal::add_hline(box.Min.X + 1.0F, box.Max.X - 1.0F, box.Min.Y + 1.0F, item.Hovered ? Color {1.0F, 1.0F, 1.0F, 0.12F} : Color {1.0F, 1.0F, 1.0F, 0.05F});
    widget_internal::add_soft_outline(box, 0.0F, border);

    if (*value) {
        const Color mark {1.0F, 1.0F, 1.0F, 0.96F};
        widget_internal::add_line({box.Min.X + 4.4F * scale, box.Min.Y + 8.3F * scale}, {box.Min.X + 7.0F * scale, box.Min.Y + 10.8F * scale}, mark, 1.35F * scale);
        widget_internal::add_line({box.Min.X + 7.0F * scale, box.Min.Y + 10.8F * scale}, {box.Min.X + 12.0F * scale, box.Min.Y + 5.7F * scale}, mark, 1.35F * scale);
    }

    const Color text_color = *value ? theme.Text : theme.TextSecondary;
    widget_internal::add_text({{box.Max.X + gap, bounds.Min.Y}, bounds.Max}, label, text_color);
