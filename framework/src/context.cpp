...
std::uint64_t hash_bytes(std::uint64_t seed, const void *data,
                         std::size_t size) {
...
bool HasContext() { return current != nullptr; }
...
void BeginFrame(const InputState &Input) {
  ...
  if (ctx.last_begin.time_since_epoch().count() != 0) {
    ctx.Stats.DeltaSeconds =
        std::chrono::duration<double>(now - ctx.last_begin).count();
...
void EndFrame() {
  context &ctx = require_context();
  ctx.combined_draw.Commands.clear();
  ctx.combined_draw.Commands.reserve(ctx.background_draw.Commands.size() +
                                     ctx.main_draw.Commands.size() +
                                     ctx.foreground_draw.Commands.size());
  ctx.combined_draw.Commands.insert(ctx.combined_draw.Commands.end(),
                                    ctx.background_draw.Commands.begin(),
                                    ctx.background_draw.Commands.end());
  ctx.combined_draw.Commands.insert(ctx.combined_draw.Commands.end(),
                                    ctx.main_draw.Commands.begin(),
                                    ctx.main_draw.Commands.end());
  ctx.combined_draw.Commands.insert(ctx.combined_draw.Commands.end(),
                                    ctx.foreground_draw.Commands.begin(),
                                    ctx.foreground_draw.Commands.end());
  ctx.Stats.FrameSeconds =
...
float MaxFps() { return require_context().MaxFps; }
...
void LimitFrameRate() {
  ...
  const auto target =
      std::chrono::duration<double>(1.0 / static_cast<double>(ctx.MaxFps));
...
const InputState &Input() { return require_context().Input; }

const DrawData &Draw() { return require_context().combined_draw; }

const Statistics &Stats() { return require_context().Stats; }

DrawData &BackgroundRenderer() { return require_context().background_draw; }

DrawData &ForegroundRenderer() { return require_context().foreground_draw; }

DrawData &MainRenderer() { return require_context().main_draw; }
...
std::uint64_t FrameIndex() { return require_context().FrameIndex; }

const Style &CurrentStyle() { return require_context().theme; }

void SetStyle(const Style &value) { require_context().theme = value; }
