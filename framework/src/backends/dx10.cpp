...
constexpr char shader_source[] =
    ...
    "    ps_in output;"
    "    float2 clip = float2((input.position.x / viewport.x) * 2.0f - 1.0f, "
    "1.0f - (input.position.y / viewport.y) * 2.0f);"
...
void push_rect(std::vector<vertex> &vertices,
  ...
  const vertex a{{bounds.Min.X, bounds.Min.Y},
                 {tint.R, tint.G, tint.B, tint.A}};
  const vertex b{{bounds.Max.X, bounds.Min.Y},
                 {tint.R, tint.G, tint.B, tint.A}};
  const vertex c{{bounds.Max.X, bounds.Max.Y},
                 {tint.R, tint.G, tint.B, tint.A}};
  const vertex d{{bounds.Min.X, bounds.Max.Y},
                 {tint.R, tint.G, tint.B, tint.A}};
...
void push_line_rect(std::vector<vertex> &vertices,
                    std::vector<std::uint32_t> &indices,
                    std::vector<render_batch> &batches, Rect bounds, Rect clip,
                    Color tint, float thickness) {
  push_rect(vertices, indices, batches,
            {bounds.Min, {bounds.Max.X, bounds.Min.Y + thickness}}, clip, tint);
  push_rect(vertices, indices, batches,
            {{bounds.Min.X, bounds.Max.Y - thickness}, bounds.Max}, clip, tint);
  push_rect(vertices, indices, batches,
            {bounds.Min, {bounds.Min.X + thickness, bounds.Max.Y}}, clip, tint);
  push_rect(vertices, indices, batches,
            {{bounds.Max.X - thickness, bounds.Min.Y}, bounds.Max}, clip, tint);
}
...
Dx10Renderer::Dx10Renderer(Window &target) : Window_(&target) {
  if (CreateDevice()) {
    CreateRenderTarget();
    CreatePipeline();
    CreateTextPipeline();
...
Dx10Renderer::~Dx10Renderer() { Release(); }

bool Dx10Renderer::Valid() const {
  return Device_ != nullptr && SwapChain_ != nullptr &&
         RenderTarget_ != nullptr;
}

void Dx10Renderer::Resize(int Width, int Height) {
...
void Dx10Renderer::Render(const DrawData &data) {
  if (!Valid()) {
    return;
  }
  ...
  Device_->OMSetRenderTargets(1, &RenderTarget_, nullptr);
  Device_->ClearRenderTargetView(RenderTarget_, clear_color);
  ...
  auto flush_geometry = [&]() {
    ...
    if (!EnsureVertexCapacity(static_cast<UINT>(vertices.size())) ||
        !EnsureIndexCapacity(static_cast<UINT>(indices.size()))) {
      vertices.clear();
      indices.clear();
      batches.clear();
      return;
    ...
    void *mapped{};
    if (FAILED(VertexBuffer_->Map(D3D10_MAP_WRITE_DISCARD, 0, &mapped))) {
      vertices.clear();
      indices.clear();
      batches.clear();
      return;
    ...
    if (FAILED(IndexBuffer_->Map(D3D10_MAP_WRITE_DISCARD, 0, &mapped))) {
      vertices.clear();
      indices.clear();
      batches.clear();
      return;
    ...
    Device_->IASetIndexBuffer(IndexBuffer_, DXGI_FORMAT_R32_UINT, 0);
    ...
    for (const render_batch &batch : batches) {
      ...
      Device_->DrawIndexed(batch.IndexCount, batch.IndexStart, 0);
    }
  ...
  constants constant_data{{static_cast<float>(Window_->Width()),
                           static_cast<float>(Window_->Height())},
                          {0.0F, 0.0F}};
  Device_->UpdateSubresource(ConstantBuffer_, 0, nullptr, &constant_data, 0, 0);
  ...
  Device_->OMSetBlendState(BlendState_, blend_factor, 0xFFFFFFFF);
  Device_->IASetInputLayout(InputLayout_);
  Device_->IASetPrimitiveTopology(D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
  Device_->VSSetShader(VertexShader_);
  Device_->VSSetConstantBuffers(0, 1, &ConstantBuffer_);
  Device_->PSSetShader(PixelShader_);
...
void Dx10Renderer::Present(bool VerticalSync) {
  if (SwapChain_ != nullptr) {
    SwapChain_->Present(VerticalSync ? 1U : 0U, 0);
  }
}

bool Dx10Renderer::CreateDevice() {
  ...
  HRESULT result = D3D10CreateDeviceAndSwapChain1(
      nullptr, D3D10_DRIVER_TYPE_HARDWARE, nullptr, create_Device_flags,
      D3D10_FEATURE_LEVEL_10_1, D3D10_1_SDK_VERSION, &SwapChain_desc,
      &SwapChain_, &Device_);

  if (FAILED(result)) {
    result = D3D10CreateDeviceAndSwapChain1(
        nullptr, D3D10_DRIVER_TYPE_HARDWARE, nullptr, create_Device_flags,
        D3D10_FEATURE_LEVEL_10_0, D3D10_1_SDK_VERSION, &SwapChain_desc,
        &SwapChain_, &Device_);
  }

  if (FAILED(result)) {
    result = D3D10CreateDeviceAndSwapChain1(
        nullptr, D3D10_DRIVER_TYPE_WARP, nullptr, create_Device_flags,
        D3D10_FEATURE_LEVEL_10_1, D3D10_1_SDK_VERSION, &SwapChain_desc,
        &SwapChain_, &Device_);
  }
...
bool Dx10Renderer::CreateRenderTarget() {
  ID3D10Texture2D *back_buffer = nullptr;
  HRESULT result = SwapChain_->GetBuffer(
      0, __uuidof(ID3D10Texture2D), reinterpret_cast<void **>(&back_buffer));
  if (FAILED(result)) {
    return false;
  }

  result =
      Device_->CreateRenderTargetView(back_buffer, nullptr, &RenderTarget_);
  release_if_set(back_buffer);
  return SUCCEEDED(result);
}

bool Dx10Renderer::CreatePipeline() {
  ID3DBlob *vertex_blob = nullptr;
  ID3DBlob *pixel_blob = nullptr;
  ID3DBlob *error_blob = nullptr;

  HRESULT result =
      D3DCompile(shader_source, sizeof(shader_source), nullptr, nullptr,
                 nullptr, "vs_main", "vs_4_0", 0, 0, &vertex_blob, &error_blob);
  ...
  result =
      D3DCompile(shader_source, sizeof(shader_source), nullptr, nullptr,
                 nullptr, "ps_main", "ps_4_0", 0, 0, &pixel_blob, &error_blob);
  release_if_set(error_blob);
  if (FAILED(result)) {
  ...
  result =
      Device_->CreateVertexShader(vertex_blob->GetBufferPointer(),
                                  vertex_blob->GetBufferSize(), &VertexShader_);
  if (FAILED(result)) {
    release_if_set(vertex_blob);
  ...
  result =
      Device_->CreatePixelShader(pixel_blob->GetBufferPointer(),
                                 pixel_blob->GetBufferSize(), &PixelShader_);
  if (FAILED(result)) {
    release_if_set(vertex_blob);
    release_if_set(pixel_blob);
    return false;
...
bool Dx10Renderer::CreateTextPipeline() {
  ...
  HRESULT result = SwapChain_->GetBuffer(0, __uuidof(IDXGISurface),
  ...
  result = D2DFactory_->CreateDxgiSurfaceRenderTarget(surface, &properties,
                                                      &D2DRenderTarget_);
  release_if_set(surface);
  return SUCCEEDED(result);
}

bool Dx10Renderer::EnsureVertexCapacity(UINT capacity) {
  if (VertexBuffer_ != nullptr && VertexCapacity_ >= capacity) {
    return true;
  }

  release_if_set(VertexBuffer_);
  VertexCapacity_ = std::max<UINT>(
      capacity, VertexCapacity_ == 0 ? 1024 : VertexCapacity_ * 2);
...
bool Dx10Renderer::EnsureIndexCapacity(UINT capacity) {
  ...
  IndexCapacity_ =
      std::max<UINT>(capacity, IndexCapacity_ == 0 ? 2048 : IndexCapacity_ * 2);
...
IDWriteTextFormat *Dx10Renderer::TextFormat(const DrawCommand &command) {
...
void Dx10Renderer::RenderText(const DrawData &data) {
  ...
  for (const DrawCommand &command : data.Commands) {
    ...
    HRESULT result =
        D2DRenderTarget_->CreateSolidColorBrush(to_d2d(command.Tint), &brush);
    if (FAILED(result)) {
      continue;
    }

    const Rect clip_rect =
        effective_clip(command.Clip, Window_->Width(), Window_->Height());
    const Rect clipped_bounds = clipped(command.Bounds, clip_rect);
    if (clipped_bounds.Max.X <= clipped_bounds.Min.X ||
        clipped_bounds.Max.Y <= clipped_bounds.Min.Y) {
      release_if_set(brush);
      continue;
    }

    const std::wstring Text = widen(command.Text);
    const D2D1_RECT_F clip = D2D1::RectF(clip_rect.Min.X, clip_rect.Min.Y,
                                         clip_rect.Max.X, clip_rect.Max.Y);
    const D2D1_RECT_F bounds =
        D2D1::RectF(command.Bounds.Min.X, command.Bounds.Min.Y,
                    command.Bounds.Max.X, command.Bounds.Max.Y);
    D2DRenderTarget_->PushAxisAlignedClip(clip,
                                          D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
    D2DRenderTarget_->DrawText(Text.c_str(), static_cast<UINT32>(Text.size()),
                               format, bounds, brush);
...
void Dx10Renderer::ReleaseRenderTarget() {
  release_if_set(D2DRenderTarget_);
  release_if_set(RenderTarget_);
}

void Dx10Renderer::Release() {
