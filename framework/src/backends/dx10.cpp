...
Dx10Renderer::Dx10Renderer(Window& target)
    : Window_(&target)
{
    if (CreateDevice()) {
        CreateRenderTarget();
...
{
    return Device_ != nullptr && SwapChain_ != nullptr && RenderTarget_ != nullptr;
}
...
bool Dx10Renderer::CreateRenderTarget()
{
    ...
    HRESULT result = SwapChain_->GetBuffer(0, __uuidof(ID3D10Texture2D), reinterpret_cast<void**>(&back_buffer));
    if (FAILED(result)) {
        return false;
    }
...
bool Dx10Renderer::CreatePipeline()
{
    ...
    result = Device_->CreateVertexShader(vertex_blob->GetBufferPointer(), vertex_blob->GetBufferSize(), &VertexShader_);
    if (FAILED(result)) {
        release_if_set(vertex_blob);
    ...
    result = Device_->CreatePixelShader(pixel_blob->GetBufferPointer(), pixel_blob->GetBufferSize(), &PixelShader_);
    if (FAILED(result)) {
        release_if_set(vertex_blob);
        release_if_set(pixel_blob);
        return false;
