...
Dx11Renderer::Dx11Renderer(Window &target) : Window_(&target) {
  if (CreateDevice()) {
    CreateRenderTarget();
    CreatePipeline();
    CreateTextPipeline();
...
Dx11Renderer::~Dx11Renderer() { Release(); }

bool Dx11Renderer::Valid() const {
  return Device_ != nullptr && Context_ != nullptr && SwapChain_ != nullptr &&
...
void Dx11Renderer::Render(const DrawData &data) {
  ...
  auto flush_geometry = [&]() {
    ...
    if (FAILED(Context_->Map(VertexBuffer_, 0, D3D11_MAP_WRITE_DISCARD, 0,
                             &mapped))) {
      vertices.clear();
      indices.clear();
      batches.clear();
    ...
    if (FAILED(Context_->Map(IndexBuffer_, 0, D3D11_MAP_WRITE_DISCARD, 0,
                             &mapped))) {
      vertices.clear();
      indices.clear();
      batches.clear();
    ...
    std::memcpy(mapped.pData, indices.data(),
  ...
  constants constant_data{{static_cast<float>(Window_->Width()),
                          ...
                          {0.0F, 0.0F}};
  Context_->UpdateSubresource(ConstantBuffer_, 0, nullptr, &constant_data, 0,
                              0);
...
bool Dx11Renderer::CreateDevice() {
  ...
  const HRESULT result = D3D11CreateDeviceAndSwapChain(
      ...
      &SwapChain_desc, &SwapChain_, &Device_, &feature_level, &Context_);
