...
Window::Window(const WindowDesc &desc)
  ...
  Handle_ = CreateWindowExW(0, class_name, desc.Title.c_str(),
...
void Window::CancelNextPoll() { CancelNextPoll_ = true; }
...
void Window::SetWndProcHook(WndProcHook hook) { Hook_ = std::move(hook); }

HWND Window::NativeHandle() const { return Handle_; }

int Window::Width() const { return Width_; }

int Window::Height() const { return Height_; }

bool Window::CloseRequested() const { return CloseRequested_; }

LRESULT CALLBACK Window::WndProc(HWND handle, UINT message, WPARAM wparam,
                                 LPARAM lparam) {
  ...
  if (self != nullptr) {
    if (self->Hook_) {
      if (std::optional<LRESULT> result =
              self->Hook_(handle, message, wparam, lparam)) {
        return *result;
      }
    }
...
LRESULT Window::HandleMessage(UINT message, WPARAM wparam, LPARAM lparam) {
  ...
  case WM_MOUSEMOVE: {
    ...
    Input_.MousePosition = {static_cast<float>(GET_X_LPARAM(lparam)),
                            static_cast<float>(GET_Y_LPARAM(lparam))};
    Input_.MouseDelta = {
        Input_.MouseDelta.X + Input_.MousePosition.X - previous.X,
        Input_.MouseDelta.Y + Input_.MousePosition.Y - previous.Y};
    return 0;
  }

  case WM_MOUSEWHEEL:
    Input_.MouseWheel += static_cast<float>(GET_WHEEL_DELTA_WPARAM(wparam)) /
