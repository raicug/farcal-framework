...
inline bool IsDemoBindingDown(int binding) {
  ...
  if (binding >= 256) {
    ...
    return mouse >= 0 && mouse < static_cast<int>(input.MouseDown.size()) &&
           input.MouseDown[static_cast<std::size_t>(mouse)];
  }

  return binding < static_cast<int>(input.KeyDown.size()) &&
         input.KeyDown[static_cast<std::size_t>(binding)];
}
...
inline void RenderDrawLayerTest(const Window &window) {
  ...
  ForegroundRenderer().Commands.push_back({
      ...
      .Bounds = {{foreground.Min.X + 8.0F * scale, foreground.Min.Y},
                 foreground.Max},
...
inline void RenderComponentDemo(ComponentDemoState &state, Window &window,
  ...
  std::snprintf(frameText, sizeof(frameText), "UI frame %.2f ms",
                stats.FrameSeconds * 1000.0);
  std::snprintf(drawText, sizeof(drawText), "Draw commands %zu",
                stats.DrawCommandCount);
  std::snprintf(memoryText, sizeof(memoryText), "Memory %.1f MB",
                static_cast<double>(stats.MemoryWorkingSet) /
                    (1024.0 * 1024.0));
  std::snprintf(directClickText, sizeof(directClickText),
                "Direct button clicks %d", state.DirectClicks);
  std::snprintf(callbackClickText, sizeof(callbackClickText),
                "Callback clicks %d", state.CallbackClicks);
  ...
  Frame([&] {
    WindowPanel("Farcal Component Test", [&] {
      ...
      TextSecondary(
          "Shared smoke window covering the public immediate-mode controls.");
      Separator();
      ...
      if (BeginTabs("Component Tabs", &state.ComponentTab, componentTabs)) {
        if (state.ComponentTab == 0) {
          SectionText("Buttons");
          PrimaryButton(
              state.PrimaryEnabled ? "Primary Enabled" : "Enable Primary", [&] {
                state.PrimaryEnabled = !state.PrimaryEnabled;
                ++state.CallbackClicks;
              });
          SetTooltip("Toggles the primary action state and increments callback "
                     "clicks.");
          TextSecondary(IsItemActive()    ? "Primary active"
                        : IsItemHovered() ? "Primary hovered"
                        : IsItemFocused() ? "Primary focused"
                                          : "Primary idle");
          ...
          SetTooltip(
              "Queues or clears a sample task using the direct button API.");
          TextSecondary(IsItemActive()    ? "Button active"
                        : IsItemHovered() ? "Button hovered"
                        : IsItemFocused() ? "Button focused"
                                          : "Button idle");

          Group([&] {
            SetNextItemWidth(104.0F);
            Button("Left", [&] { ++state.CallbackClicks; });
            SameLine();
            SetNextItemWidth(104.0F);
            Button("Right", [&] { ++state.CallbackClicks; });
          });
          std::snprintf(hoverText, sizeof(hoverText), "Group is %s",
                        IsItemHovered() ? "hovered" : "idle");
          Indent();
          ...
          Spacing();
          SectionText("Toggles");
          PushStyleVar(StyleVar::AntiAliasing,
                       state.Diagnostics ? 1.65F : 1.0F);
          ...
          SetTooltip(
              "Click to capture a key. Right-click to choose Toggle or Hold.");
          ...
          PushStyleColor(StyleColor::Text, state.PrimaryEnabled
                                               ? Color::Rgb(82, 210, 115)
                                               : Color::Rgb(232, 178, 76));
          Text(state.PrimaryEnabled ? "Primary feature is enabled"
                                    : "Primary feature is disabled");
          PopStyleColor();
          ...
          Button("Styled Button", [&] { ++state.CallbackClicks; });
          PopStyleColor();
          PopStyleColor();
          PopStyleColor();
          ...
          PushStyleColor(
              StyleColor::ButtonPrimaryHovered,
              Color::Rgba(
                  (std::min)(255,
                             static_cast<int>(state.AccentPreview.R * 255.0F) +
                                 24),
                  (std::min)(255,
                             static_cast<int>(state.AccentPreview.G * 255.0F) +
                                 24),
                  (std::min)(255,
                             static_cast<int>(state.AccentPreview.B * 255.0F) +
                                 24)));
          PushStyleColor(
              StyleColor::ButtonPrimaryActive,
              Color::Rgba(
                  (std::max)(0,
                             static_cast<int>(state.AccentPreview.R * 255.0F) -
                                 24),
                  ...
                  (std::max)(0,
                             static_cast<int>(state.AccentPreview.B * 255.0F) -
                                 24)));
          PrimaryButton("Preview Color", [&] { ++state.CallbackClicks; });
          PopStyleColor();
          PopStyleColor();
          PopStyleColor();
          ...
          Button("Stop Polling Next Frame", [&] { window.CancelNextPoll(); });
          ...
          SetTooltip("Shows a viewport overlay that verifies background, main, "
          ...
          List("Asset List",
               [&] {
                 for (int index = 0; index < 8; ++index) {
                   char label[64]{};
                   std::snprintf(label, sizeof(label), "%.*s_ListItem_%02d",
                                 static_cast<int>(assetPrefix.size()),
                                 assetPrefix.data(), index + 1);
          ...
          for (int index = 0; index < 24; ++index) {
            ...
            char label[64]{};
            std::snprintf(label, sizeof(label), "%.*s_TestAsset_%02d",
                          static_cast<int>(assetPrefix.size()),
                          assetPrefix.data(), index + 1);
    ...
    if (BeginWindow("Direct BeginWindow API")) {
      ...
      Text(state.Vsync ? "Presentation waits for VSync"
                       : "Presentation presents immediately");
    }
    EndWindow();
  });
