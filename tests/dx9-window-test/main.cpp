...
int main() {
  try {
    ...
    farcal::Dx9Renderer renderer(Window);
    if (!renderer.Valid()) {
      std::fprintf(stderr, "failed to initialize DX9 renderer\n");
    ...
    while (Window.PollEvents()) {
      ...
      farcal::tests::RenderComponentDemo(demo, Window, "DirectX 9", "DX9");
